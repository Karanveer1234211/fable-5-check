...
def _join_macro_features(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    Memory-safe: maps each macro column onto the panel IN PLACE via a per-date
    lookup, instead of panel.merge() -- which copied the whole 200+ col panel and
    consolidated its ~183 float64 columns into one 5.8 GiB block (the OOM).
    """
    ...
    try:
        ...
        if not macro_cols:
            print("[Macro] Joined 0 macro features (no M_ columns in cache).")
            return panel

        # AUDIT-FIX 2: same-day, tz-safe join date (preserves the IST wall-clock
        # date; the old tz_convert order shifted IST midnight to the prior UTC
        # date and joined yesterday's macro). Computed locally -- not added as a
        # panel column -- so we never merge/copy the full panel.
        panel_date = pd.to_datetime(panel["date"])
        if getattr(panel_date.dt, "tz", None) is not None:
            jd = panel_date.dt.tz_localize(None).dt.normalize()
        else:
            jd = panel_date.dt.normalize()

        # Per-date lookup with a UNIQUE index (keep last on duplicate dates).
        m = (macro.drop_duplicates("date", keep="last")
                  .set_index("date")[macro_cols])

        # Map each macro column ONTO the panel in place (no full-panel copy).
        # float32 halves memory; macro values (vix, returns, z-scores) need no more.
        for c in macro_cols:
            panel[c] = jd.map(m[c]).astype("float32")
            panel[c] = panel.groupby("symbol")[c].ffill()

        # AUDIT-FIX 2b: coverage across ALL macro columns, memory-safe (running
        # AND over a single bool array -- no pd.concat of N full-length frames).
        full = np.ones(len(panel), dtype=bool)
        for c in macro_cols:
            full &= panel[c].notna().to_numpy()
        print(f"[Macro] Joined {len(macro_cols)} macro features in place; "
              f"full coverage on {int(full.sum()):,} of {len(panel):,} rows")
        return panel
    except Exception as e:
...
def build_5d_rank_quant_labels(panel: pd.DataFrame, ev_target: str = "cc") -> pd.DataFrame:
    # MEMORY FIX: this function only ADDS label columns (never mutates existing
    # ones), so a deep copy of all ~195 float cols is pure waste -- it forces a
    # single consolidated (195 x N) block (~3.1 GiB) that OOMs on this box. A
    # shallow copy shares existing blocks, lets us append new columns, and leaves
    # the caller\'s panel untouched.
