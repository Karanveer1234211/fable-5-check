...
ap.add_argument("--tp-pct", type=float, default=0.15, help="bracket take-profit (fraction, e.g. 0.15)")
ap.add_argument("--sl-stop-pct", type=float, default=0.15, help="bracket catastrophe stop (fraction)")
...
for r in wl.itertuples(index=False):
    ...
    # THE BRACKET you actually trade: +tp / -sl GTT walked day by day.
    # Conservative: if both rails touch the same day, assume the SL filled first.
    br_ret = float(closes[-1]) / entry - 1.0; br_exit = "horizon"; br_days = H
    up = entry * (1 + args.tp_pct); dn = entry * (1 - args.sl_stop_pct)
    fh = fut["high"].values.astype(float); fl = fut["low"].values.astype(float)
    for k in range(H):
        if fl[k] <= dn:
            br_ret = -args.sl_stop_pct; br_exit = "SL"; br_days = k + 1; break
        if fh[k] >= up:
            br_ret = args.tp_pct; br_exit = "TP"; br_days = k + 1; break
    rec.update(status="resolved",
               ...
               bracket_ret=round(br_ret, 4),
               bracket_exit=br_exit,
               bracket_days=br_days)
...
if len(done) >= 5:
    ...
    def card(dd, lab):
        ...
        if "bracket_ret" in dd.columns:
            br = pd.to_numeric(dd["bracket_ret"], errors="coerce")
            ex = dd["bracket_exit"].astype(str)
            tp = (ex == "TP").mean() * 100; slh = (ex == "SL").mean() * 100
            hz = (ex == "horizon").mean() * 100
            edge = (br.mean() - dd.ret_5.mean()) * 100
            print(f"  {'':28} BRACKET(+{args.tp_pct:.0%}/-{args.sl_stop_pct:.0%})  "
                  f"meanRet {br.mean()*100:+6.2f}%  TP {tp:4.1f}%  SL {slh:4.1f}%  "
                  f"hz {hz:4.1f}%  vs hold {edge:+5.2f}%")
