...
def load_panel(path):
    ...
    def _swing_flags(gg):
        lo = gg["low"].to_numpy(float); hi = gg["high"].to_numpy(float)
        n = len(gg)
        is_lo = np.zeros(n, bool); is_hi = np.zeros(n, bool)
        for t in range(K, n - K):
            w_lo = lo[t-K:t+K+1]; w_hi = hi[t-K:t+K+1]
            if lo[t] == w_lo.min() and (w_lo.argmin() == K): is_lo[t] = True
            if hi[t] == w_hi.max() and (w_hi.argmax() == K): is_hi[t] = True
        # walk forward; at each bar carry the last TWO confirmed pivots. A pivot at
        # bar p is only "known" from bar p+K onward (needs K future bars to confirm).
        last2_lo = [np.nan, np.nan]; last2_hi = [np.nan, np.nan]
        hl = np.zeros(n, np.int8)   # 1 = last two confirmed swing-lows are RISING
        lh = np.zeros(n, np.int8)   # 1 = last two confirmed swing-highs are FALLING
        confirm_lo_at = {p + K: lo[p] for p in range(n) if is_lo[p] and p + K < n}
        confirm_hi_at = {p + K: hi[p] for p in range(n) if is_hi[p] and p + K < n}
        for t in range(n):
            if t in confirm_lo_at:
                last2_lo = [last2_lo[1], confirm_lo_at[t]]
            if t in confirm_hi_at:
                last2_hi = [last2_hi[1], confirm_hi_at[t]]
        ...
        return pd.DataFrame({"higher_lows": hl, "lower_highs": lh}, index=gg.index)
    sw = g.apply(_swing_flags)
    if isinstance(sw.index, pd.MultiIndex):
        sw = sw.reset_index(level=0, drop=True)
    df["higher_lows"] = sw["higher_lows"].astype("int8")
    df["lower_highs"] = sw["lower_highs"].astype("int8")
    # "clean uptrend structure" = making higher lows AND not making lower highs
